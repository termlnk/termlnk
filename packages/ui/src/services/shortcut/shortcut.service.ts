...
import { createIdentifier, Disposable, ICommandService, IContextService, Optional, toDisposable } from '@termlnk/core';
import { Subject } from 'rxjs';
import { fromGlobalEvent } from '../../common/event';
...
export class ShortcutService extends Disposable implements IShortcutService {
  ...
  getAllShortcuts(): IShortcutItem[] {
    return Array.from(this._shortCutMapping.values(), (v) => [...v.values()])
      .flat();
  }
  ...
  dispatch(e: KeyboardEvent): IShortcutItem<object> | undefined {
    ...
    // Should get the container element of the instance and see if
    ...
    // if the target is not focused on the instance we should ignore the keyboard event.
    // Maybe the user has forcibly disabled the shortcut keys, and the shortcut keys should not be processed at this time.
    if (this._forceEscaped || this._forceDisabled) {
      return;
    }
    ...
    if (
      ...
      && !this._layoutService.checkElementInCurrentContainers(target)
    ) {
      return;
    }
    ...
    const candidateShortcut = [...shortcuts]
...
function isIMEKeyboardEvent(e: KeyboardEvent): boolean {
