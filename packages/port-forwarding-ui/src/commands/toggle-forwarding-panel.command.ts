...
export const ToggleForwardingPanelCommand: ICommand = {
  id: 'port-forwarding-ui.command.toggle-forwarding-panel',
  handler: (accessor: IAccessor): boolean => {
    ...
    if (sideTabBarService.active === ToggleForwardingPanelCommand.id) {
      resizableService.toggle('left');
    } else {
      sideTabBarService.activate(ToggleForwardingPanelCommand.id);
    }
