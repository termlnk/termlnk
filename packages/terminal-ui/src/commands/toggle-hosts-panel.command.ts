...
export const ToggleHostsPanelCommand: ICommand = {
  id: 'terminal-ui.command.toggle-hosts-panel',
  handler: (accessor: IAccessor): boolean => {
    ...
    if (sideTabBarService.active === ToggleHostsPanelCommand.id) {
      resizableService.toggle('left');
    } else {
      sideTabBarService.activate(ToggleHostsPanelCommand.id);
    }
