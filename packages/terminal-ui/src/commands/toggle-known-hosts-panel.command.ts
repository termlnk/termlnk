...
export const ToggleKnownHostsPanelCommand: ICommand = {
  id: 'terminal-ui.command.toggle-known-hosts-panel',
  handler: (accessor: IAccessor): boolean => {
    ...
    if (sideTabBarService.active === ToggleKnownHostsPanelCommand.id) {
      resizableService.toggle('left');
    } else {
      sideTabBarService.activate(ToggleKnownHostsPanelCommand.id);
    }
