...
export const ToggleKeychainPanelCommand: ICommand = {
  id: 'terminal-ui.command.toggle-keychain-panel',
  handler: (accessor: IAccessor): boolean => {
    ...
    if (sideTabBarService.active === ToggleKeychainPanelCommand.id) {
      resizableService.toggle('left');
    } else {
      sideTabBarService.activate(ToggleKeychainPanelCommand.id);
    }
