...
export const RenameHostCommand: ICommand = {
  id: 'terminal-ui.command.rename-host',
  handler: (accessor: IAccessor) => {
    const hostExplorerService = accessor.get(IHostExplorerService);
    const focused = hostExplorerService.getFocusedHost();
    if (!focused) {
      return false;
    }

    hostExplorerService.requestRename(focused.id);
    return true;
  },
};
