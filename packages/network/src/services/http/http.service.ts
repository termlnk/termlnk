...
import { Disposable, removeAt, toDisposable } from '@termlnk/core';
...
export class HTTPService extends Disposable {
  ...
  registerHTTPInterceptor(interceptor: IHTTPInterceptor): IDisposable {
    ...
    return toDisposable(() => removeAt(this._interceptors, interceptor));
  }
   ...
   */
  async request<T>(method: HTTPRequestMethod, url: string, options?: IRequestParams): Promise<HTTPResponse<T>> {
