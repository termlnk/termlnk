...
import type { IMcpServerCapabilities, McpConnectionStatus, McpServerConfig, McpTransportType } from '@termlnk/agent';
...
export const mcpServerEntity = sqliteTable('mcp_server', {
  ...
  transport: text('transport').notNull().$type<McpTransportType>(),
  config: text('config', { mode: 'json' }).notNull().$type<McpServerConfig>(),
  capabilities: text('capabilities', { mode: 'json' }).$type<IMcpServerCapabilities>(),
  toolCount: integer('tool_count').notNull().default(0),
  resourceCount: integer('resource_count').notNull().default(0),
  enabled: integer('enabled', { mode: 'boolean' }).notNull().default(true),
  status: text('status').notNull().default('disconnected').$type<McpConnectionStatus>(),
