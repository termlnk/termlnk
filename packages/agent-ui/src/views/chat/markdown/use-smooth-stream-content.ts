...
export type IStreamSmoothingPreset = 'realtime' | 'balanced' | 'silky';

export interface IUseSmoothStreamContentOptions {
  enabled?: boolean;
...
const PRESET_CONFIG: Record<IStreamSmoothingPreset, IPresetConfig> = {
  balanced: {
...
// RAF-driven output buffer that lags `content` by a small adaptive amount so
// upstream bursts cascade instead of collapsing into a single visible jump.
export function useSmoothStreamContent(
  content: string,
  { enabled = true, preset = 'balanced' }: IUseSmoothStreamContentOptions = {}
): string {
  ...
  const startFrameLoop = useCallback(() => {
    ...
    const tick = (ts: number) => {
      ...
      if (inputActive) {
        ...
        // Upstream stopped — drain backlog inside a bounded window.
