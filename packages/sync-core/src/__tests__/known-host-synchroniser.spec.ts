...
import type { IKnownHostEntity, ISyncRowMetaEntity, KnownHostRepository, SyncRowMetaRepository } from '@termlnk/database';
...
class FakeKnownHostRepo {
  rows: Map<string, IKnownHostEntity> = new Map();
...
function makeKnownHost(id: string, overrides: Partial<IKnownHostEntity> = {}): IKnownHostEntity {
  return {
    id,
    host: 'example.com',
    port: 22,
...
interface ITestBed {
  repo: FakeKnownHostRepo;
  rowMeta: FakeRowMetaRepo;
  outbox: FakeOutbox;
  crypto: FakeCrypto;
  syncer: KnownHostSynchroniser;
}

function createTestBed(): ITestBed {
  const repo = new FakeKnownHostRepo();
  const rowMeta = new FakeRowMetaRepo();
  const outbox = new FakeOutbox();
  const crypto = new FakeCrypto();
  const syncer = new KnownHostSynchroniser(
    repo as unknown as KnownHostRepository,
...
describe('KnownHostSynchroniser', () => {
  let bed: ITestBed;
  ...
  it('exposes resourceId = "known_host"', () => {
    expect(bed.syncer.resourceId).toBe('known_host');
  });

  it('local add enqueues an upsert mutation with encrypted full row', async () => {
    bed.syncer.start();
    bed.repo.rows.set('h1', makeKnownHost('h1', { host: 'created.local' }));
    bed.repo.changed$.next({ type: 'add', id: 'h1' });

    await flushAsync();

    expect(bed.outbox.enqueued).toHaveLength(1);
    expect(bed.outbox.enqueued[0].entityId).toBe('h1');
    expect(bed.outbox.enqueued[0].op).toBe('upsert');
  });

  it('local delete enqueues a delete mutation with null payload', async () => {
    bed.syncer.start();
    bed.repo.changed$.next({ type: 'delete', id: 'h1' });
  ...
  it('applyPatch op=put creates a row that did not exist locally', async () => {
    bed.syncer.start();
    const row = makeKnownHost('h1', { host: 'remote-create.local' });
    ...
    expect(bed.repo.rows.get('h1')?.host).toBe('remote-create.local');
    expect((await bed.rowMeta.get('known_host', 'h1'))?.version).toBe(5);
  });

  it('applyPatch op=put updates an existing row in place', async () => {
    bed.syncer.start();
    bed.repo.rows.set('h1', makeKnownHost('h1', { fingerprint: 'SHA256:old' }));
    const updated = makeKnownHost('h1', { fingerprint: 'SHA256:new' });
    const patch: ISyncPatchItem = {
      op: 'put',
      resource: 'known_host',
      entityId: 'h1',
    ...
    expect(bed.repo.rows.get('h1')?.fingerprint).toBe('SHA256:new');
  });

  it('applyPatch op=del removes the row and its row meta', async () => {
    bed.syncer.start();
    bed.repo.rows.set('h1', makeKnownHost('h1'));
    await bed.rowMeta.upsert({ resource: 'known_host', entityId: 'h1', version: 1, updatedAt: Date.now() });

    await bed.syncer.applyPatch([{ op: 'del', resource: 'known_host', entityId: 'h1', payload: null, version: 2 }]);

    expect(bed.repo.rows.has('h1')).toBe(false);
    expect(await bed.rowMeta.get('known_host', 'h1')).toBeNull();
  });

  it('changed$ events emitted while applyPatch is running do NOT fan back to outbox', async () => {
    bed.syncer.start();
    const row = makeKnownHost('h1', { host: 'remote' });
    const patch: ISyncPatchItem = {
      op: 'put',
      resource: 'known_host',
      entityId: 'h1',
  ...
  it('buildInitialSnapshot enqueues one upsert per existing row', async () => {
    bed.repo.rows.set('h1', makeKnownHost('h1'));
    bed.repo.rows.set('h2', makeKnownHost('h2'));
  ...
  it('buildInitialSnapshot skips rows that already have a sync_row_meta entry', async () => {
    bed.repo.rows.set('h1', makeKnownHost('h1'));
    bed.repo.rows.set('h2', makeKnownHost('h2'));
    await bed.rowMeta.upsert({ resource: 'known_host', entityId: 'h1', version: 7, updatedAt: Date.now() });

    const snapshot = await bed.syncer.buildInitialSnapshot();

    expect(snapshot).toHaveLength(1);
    expect(bed.outbox.enqueued).toHaveLength(1);
    expect(bed.outbox.enqueued[0].entityId).toBe('h2');
  });
