...
import type { ISshKeyEntity, ISyncRowMetaEntity, SshKeyRepository, SyncRowMetaRepository } from '@termlnk/database';
...
class FakeSshKeyRepo {
  rows: Map<string, ISshKeyEntity> = new Map();
...
function makeKey(id: string, overrides: Partial<ISshKeyEntity> = {}): ISshKeyEntity {
  return {
    id,
    label: 'demo-key',
    algorithm: 'ed25519',
...
interface ITestBed {
  ...
  outbox: FakeOutbox;
  crypto: FakeCrypto;
  syncer: SshKeySynchroniser;
}

function createTestBed(): ITestBed {
  const repo = new FakeSshKeyRepo();
  const rowMeta = new FakeRowMetaRepo();
  const outbox = new FakeOutbox();
  const crypto = new FakeCrypto();
  const syncer = new SshKeySynchroniser(
    repo as unknown as SshKeyRepository,
...
describe('SshKeySynchroniser', () => {
  ...
  it('exposes resourceId = "ssh_key"', () => {
    expect(bed.syncer.resourceId).toBe('ssh_key');
  });

  it('local add enqueues an upsert mutation with encrypted full row', async () => {
    bed.syncer.start();
    bed.repo.rows.set('k1', makeKey('k1', { label: 'created' }));
    bed.repo.changed$.next({ type: 'add', id: 'k1' });

    await flushAsync();

    expect(bed.outbox.enqueued).toHaveLength(1);
    expect(bed.outbox.enqueued[0].entityId).toBe('k1');
    expect(bed.outbox.enqueued[0].op).toBe('upsert');
  });

  it('local delete enqueues a delete mutation with null payload', async () => {
    bed.syncer.start();
    bed.repo.changed$.next({ type: 'delete', id: 'k1' });
  ...
  it('applyPatch op=put creates a row that did not exist locally', async () => {
    bed.syncer.start();
    const row = makeKey('k1', { label: 'remote-create' });
    ...
    expect(bed.repo.rows.get('k1')?.label).toBe('remote-create');
    expect((await bed.rowMeta.get('ssh_key', 'k1'))?.version).toBe(5);
  });

  it('applyPatch op=put updates an existing row in place', async () => {
    bed.syncer.start();
    bed.repo.rows.set('k1', makeKey('k1', { label: 'original' }));
    const updated = makeKey('k1', { label: 'rotated' });
    const patch: ISyncPatchItem = {
      op: 'put',
      resource: 'ssh_key',
      entityId: 'k1',
    ...
    expect(bed.repo.rows.get('k1')?.label).toBe('rotated');
  });

  it('applyPatch op=del removes the row and its row meta', async () => {
    bed.syncer.start();
    bed.repo.rows.set('k1', makeKey('k1'));
    await bed.rowMeta.upsert({ resource: 'ssh_key', entityId: 'k1', version: 1, updatedAt: Date.now() });

    await bed.syncer.applyPatch([{ op: 'del', resource: 'ssh_key', entityId: 'k1', payload: null, version: 2 }]);

    expect(bed.repo.rows.has('k1')).toBe(false);
    expect(await bed.rowMeta.get('ssh_key', 'k1')).toBeNull();
  });

  it('changed$ events emitted while applyPatch is running do NOT fan back to outbox', async () => {
    bed.syncer.start();
    const row = makeKey('k1', { label: 'remote' });
    const patch: ISyncPatchItem = {
      op: 'put',
      resource: 'ssh_key',
      entityId: 'k1',
  ...
  it('buildInitialSnapshot enqueues one upsert per existing row', async () => {
    bed.repo.rows.set('k1', makeKey('k1'));
    bed.repo.rows.set('k2', makeKey('k2'));
  ...
  it('buildInitialSnapshot skips rows that already have a sync_row_meta entry', async () => {
    bed.repo.rows.set('k1', makeKey('k1'));
    bed.repo.rows.set('k2', makeKey('k2'));
    await bed.rowMeta.upsert({ resource: 'ssh_key', entityId: 'k1', version: 7, updatedAt: Date.now() });

    const snapshot = await bed.syncer.buildInitialSnapshot();

    expect(snapshot).toHaveLength(1);
    expect(bed.outbox.enqueued).toHaveLength(1);
    expect(bed.outbox.enqueued[0].entityId).toBe('k2');
  });
