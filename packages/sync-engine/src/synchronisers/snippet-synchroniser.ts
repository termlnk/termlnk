...
export class SnippetSynchroniser extends RxDisposable implements IResourceSynchroniser {
  ...
  constructor(
    @ISnippetSyncRepository private readonly _repo: ISnippetSyncRepository,
  ...
  private async _applyOne(item: ISyncPatchItem): Promise<void> {
    ...
    if (item.op === 'put') {
      ...
      try {
        ...
        this._logService.warn(`[SnippetSynchroniser] failed to apply put for ${item.entityId}:`, err);
      }
    }
  }
}
