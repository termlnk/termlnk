...
import { ISnippetSyncRepository, ISyncCryptoService, ISyncOutboxService, ISyncRowMetaRepository, SynchroniserStatus } from '@termlnk/sync';
import { BehaviorSubject } from 'rxjs';

const RESOURCE_ID = 'snippet' as const;
const TEXT_ENCODER = new TextEncoder();
const TEXT_DECODER = new TextDecoder();

// Row-level LWW for snippets. The whole row is encrypted under the master key for transport
// so the server stays zero-knowledge — name/command/description never leave plaintext. The
// receiver re-encrypts at-rest via the local cipher in syncUpsertRow.
export class SnippetSynchroniser extends RxDisposable implements IResourceSynchroniser {
  ...
  constructor(
    @ISnippetSyncRepository private readonly _snippetRepo: ISnippetSyncRepository,
  ...
  start(): void {
    ...
    this._started = true;
    this.disposeWithMe(
      this._snippetRepo.changed$.subscribe((event) => {
  ...
  async reconcileGhostMeta(serverEntityIds: ReadonlySet<string>): Promise<void> {
    ...
    for (const meta of all) {
      if (!serverEntityIds.has(meta.entityId)) {
        this._logService.log(`[SnippetSynchroniser] dropping ghost meta for ${meta.entityId}`);
  ...
  async buildInitialSnapshot(): Promise<ISyncMutation[]> {
    ...
    const rows = await this._snippetRepo.getList();
  ...
  private async _handleLocalChange(id: string, type: 'add' | 'update' | 'delete'): Promise<void> {
    try {
      ...
      const row = await this._snippetRepo.getById(id);
      if (!row) {
        await this._outboxService.enqueue({
          resource: RESOURCE_ID,
          op: 'delete',
      ...
      this._logService.error(`[SnippetSynchroniser] failed to enqueue mutation for ${id}:`, err);
  ...
  private async _applyOne(item: ISyncPatchItem): Promise<void> {
    if (item.op === 'clear') {
      const all = await this._snippetRepo.getList();
      for (const r of all) {
        await this._snippetRepo.delete(r.id);
      }
    ...
    if (item.entityId === null) {
      this._logService.warn('[SnippetSynchroniser] patch item missing entityId; skipping');
      return;
    }

    if (item.op === 'del') {
      await this._snippetRepo.delete(item.entityId);
    ...
    if (item.op === 'put') {
      if (!item.payload) {
        this._logService.warn(`[SnippetSynchroniser] put without payload for ${item.entityId}`);
        return;
      }
      ...
      await this._snippetRepo.syncUpsertRow(normalised);
      await this._rowMetaRepo.upsert({
        resource: RESOURCE_ID,
