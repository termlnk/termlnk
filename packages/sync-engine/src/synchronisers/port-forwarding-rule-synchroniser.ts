...
// Row-level LWW for port-forwarding rules. Only the rule definition syncs — the live tunnel
// state (open/closed) is never persisted and never enters the sync stream. The whole row is
// encrypted under the master key for transport so the server stays zero-knowledge. hostId is
// a soft reference to host with no FK; cross-resource pull order may briefly leave it
// dangling, which the consumer tolerates (rules for a not-yet-synced host stay inert).
export class PortForwardingRuleSynchroniser extends RxDisposable implements IResourceSynchroniser {
  ...
  constructor(
    @IPortForwardingRuleSyncRepository private readonly _ruleRepo: IPortForwardingRuleSyncRepository,
  ...
  start(): void {
    ...
    this._started = true;
    this.disposeWithMe(
      this._ruleRepo.changed$.subscribe((event) => {
  ...
  async buildInitialSnapshot(): Promise<ISyncMutation[]> {
    ...
    const rows = await this._ruleRepo.getList();
  ...
  private async _handleLocalChange(id: string, type: 'add' | 'update' | 'delete'): Promise<void> {
    try {
      ...
      const row = await this._ruleRepo.getById(id);
      if (!row) {
        await this._outboxService.enqueue({
          resource: RESOURCE_ID,
          op: 'delete',
  ...
  private async _applyOne(item: ISyncPatchItem): Promise<void> {
    if (item.op === 'clear') {
      const all = await this._ruleRepo.getList();
      for (const r of all) {
        await this._ruleRepo.delete(r.id);
      }
    ...
    if (item.op === 'del') {
      await this._ruleRepo.delete(item.entityId);
    ...
    if (item.op === 'put') {
      ...
      await this._ruleRepo.syncUpsertRow(normalised);
      await this._rowMetaRepo.upsert({
        resource: RESOURCE_ID,
