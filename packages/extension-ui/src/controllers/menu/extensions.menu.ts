...
export function extensionsMenuFactory(accessor: IAccessor): IMenuItem {
  const sideTabBarService = accessor.get(SideTabBarService);
  return {
    id: ToggleExtensionsPanelCommand.id,
    type: MenuItemType.BUTTON,
    componentId: EXTENSIONS_EXPLORER_NAME,
    title: 'extension-ui.menu.extensions',
    tooltip: 'extension-ui.menu.extensions',
    icon: EXTENSIONS_ICON_NAME,
    hidden$: of(false),
    activated$: sideTabBarService.active$.pipe(
      map((id) => id === ToggleExtensionsPanelCommand.id)
    ),
  };
}
