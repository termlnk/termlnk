...
export const ToggleExtensionsPanelCommand: ICommand = {
  id: 'extension-ui.command.toggle-extensions-panel',
  handler: (accessor: IAccessor): boolean => {
    ...
    if (sideTabBarService.active === ToggleExtensionsPanelCommand.id) {
      resizableService.toggle('left');
    } else {
      sideTabBarService.activate(ToggleExtensionsPanelCommand.id);
    }
