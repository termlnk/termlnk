...
// Field-level LWW metadata (config resource only). Per-subKey LWW prevents two devices
// clobbering each other's unrelated subKeys under the same config row.
export const syncFieldMetaEntity = sqliteTable('sync_field_meta', {
  ...
  primaryKey({ columns: [t.resource, t.entityId, t.field] }),
]);
