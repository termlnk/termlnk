...
// Terminal screen — P6.9-7 rebuild on @termlnk/react-native-russh.
//
// Flow:
//   1. Pull host metadata from the synced vault (read-only here; vault
//      mutations live on desktop until v1.1).
//   2. User enters password; v1.1 will read decrypted host.credential
//      from the vault when SSH-key auth lands.
//   3. MobileSshClientService.connect → startShell → writeToShell with
//      the tmux/screen resumption command from mobile-shell-resumption.
//   4. Shell output stream → injectJavaScript on the xterm webview;
//      webview keyboard input → writeToShell. Same bridge as P6.6.
...
export default function TerminalScreen() {
  ...
  const onConnect = async () => {
    ...
    try {
      ...
      await next.writeToShell(`${buildShellResumptionCommand({ hostId: host.id }).command}\r`);
      setSession(next);
      setState('connected');
    } catch (err) {
    ...
    <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      {state !== 'connected' && (
        <View style={styles.credentials}>
          ...
          <Pressable
            onPress={onConnect}
            disabled={state === 'connecting' || creds.password.length === 0 || creds.username.length === 0}
            style={({ pressed }) => [
              styles.button,
              (state === 'connecting' || creds.password.length === 0 || creds.username.length === 0) && styles.buttonDisabled,
          ...
          <Text style={styles.note}>
            Password auth in v1; SSH-key auth from the synced vault lands in v1.1.
            Host key is pinned on first connect — a fingerprint change will refuse the handshake.
          </Text>
        </View>
      )}
      {state === 'connected' && (
