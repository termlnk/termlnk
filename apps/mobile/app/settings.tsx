...
import { Activity, Cpu, Eye, FlaskConical, Gauge, Hash, Keyboard, Languages, MapPin, Moon, RefreshCw, Rows3, Search, ShieldCheck, Sparkles, SquareChevronUp, TextCursor, Timer, Type, Vibrate } from 'lucide-react-native';
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { Alert, FlatList, ScrollView, Text, useWindowDimensions, View } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useObservable, usePreferencesService, useSyncService } from '../src/core/core-context';
import { FontSizePreviewRow } from '../src/terminal/font-size-preview-row';
import { getFontLabel, getTerminalFonts, KEEP_ALIVE_OPTIONS, SCROLLBACK_OPTIONS } from '../src/terminal/terminal-config';
import { ThemeMiniCard } from '../src/terminal/theme-mini-card';
  ...
  return (
    <ScreenContainer>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 32, gap: 5 }}>
        <SectionTitle title="Terminal" first />
        <Card>
          <View>
        ...
        <Card>
          <NavRow leading={<IconTile icon={Sparkles} tone="ai" size={30} />} title="Provider & API key" onPress={() => router.push('/ai-settings')} />
        </Card>
...
function SectionTitle({ title, first }: { title: string; first?: boolean }) {
  return (
    <Text className={cn('px-1 pb-2 text-[12px] font-semibold uppercase tracking-wider text-content-tertiary', { 'pt-1': first, 'pt-5': !first })}>{title}</Text>
  );
}
