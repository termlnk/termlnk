...
function InitialAvatar({ user, size, dimension }: { user: IUserAccount | null; size: number; dimension: { width: number; height: number; borderRadius: number } }) {
  return (
    <View
...
export function UserAvatar({ user, size = 36, radius = 14 }: IUserAvatarProps) {
  ...
  return <InitialAvatar user={user} size={size} dimension={dimension} />;
}
