...
import { DEFAULT_PREFERENCES } from './mobile-preferences.service';
...
export function BiometricGate({ children }: { children: ReactNode }) {
  ...
  return (
    <View className="flex-1 items-center justify-center bg-black">
      <Lock size={40} color="#565c64" />
      <Text className="mt-4 text-[15px] text-light-grey">termlnk is locked</Text>
      <Pressable onPress={unlock} className="mt-6 rounded-xl bg-nord-blue px-6 py-3 active:opacity-80">
        <Text className="text-[15px] font-semibold text-black">Unlock</Text>
      </Pressable>
    </View>
  );
}
